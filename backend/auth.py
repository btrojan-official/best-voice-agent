...
class AuthService:
    ...
    def verify_token(token: Optional[str]) -> bool:
        ...
        try:
            ...
            if len(parts) != 2:
                logger.warning(
                    f"Invalid token format, expected 2 parts, got {len(parts)}"
                )
                return False
            ...
            logger.info(
                f"Token verification: username={username}, expected={admin_username}, result={result}"
            )
            return result
